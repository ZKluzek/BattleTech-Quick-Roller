...
function clusterAttack(){
    ...
    var locationsHitArray = [];
    for(var i = 0; i < shotsHit; i++){
        locationsHitArray.append(rollHitLocation(direction.value));
    }
    for ( hit in locationsHitArray ){
        if()
    }
    
    eachNumRolled = rollMultiple(shotsHit);
    // for( var number in eachNumRolled.values){
    //     console.log(number);
        
    // }

    for(var i = 0; i < eachNumRolled.values.length; i++){
        if(eachNumRolled.values[i] != 0){
